...
def calculate_sky(sci_data, bin_size = 64, dq_data = None, dq_good_list = [0], has_DQ = True, dq_fraction = 0.2, percentile = 50):
    ...
    image_x = data.shape[0]
    image_y = data.shape[1]
    mask_array = np.zeros((image_x, image_y))

    for index in bad_ind:
        cutout = cutouts[index]

        mask_x = cutout.bbox_original[0][0] // 64
        mask_y = cutout.bbox_original[1][0] // 64

        x1 = image_x-cutout.bbox_original[0][0]
        x2 = image_x-cutout.bbox_original[0][1]

        y1 = cutout.bbox_original[1][0]
        y2 = cutout.bbox_original[1][1]

        mask_array[x2:x1 + 1, y1:y2 + 1] = 1

    cv2.imwrite("img.png", 255-(mask_array * 255))
